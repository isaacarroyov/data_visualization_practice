...
map_circles_areas = folium.Map( location= ( df.Latitude.mean(), df.Longitude.mean() ), zoom_start= 6 )

for i in range(loc.shape[0]):
    folium.Circle(
    location = loc[i].tolist(),
    radius = np.sqrt( df['Affected area (ha)'].values[i]/np.pi )*1000, #multiply by 1000 for better visualization
    popup = df['Duration time (days)'].values[i],
    color = df['Duration time (colours)'].values[i],
    fill = True,
    fill_color = df['Duration time (colours)'].values[i]
    ).add_to(map_circles_areas)

map_circles_areas.save('maps/circles_duration_affected_area.html')
print('Saved map as circles_duration_affected_area.html')
